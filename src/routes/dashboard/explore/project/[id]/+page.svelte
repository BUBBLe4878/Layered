...
<div class="max-w-3xl mx-auto px-4 py-8 space-y-8">
  ...
  <!-- Back -->
  <a href="/dashboard/explore" class="text-sm text-gray-500 hover:text-gray-800">← Back to Explore</a>
  ...
  <div class="bg-white rounded-2xl border p-6 space-y-3">
    <div class="flex items-start justify-between gap-4 flex-wrap">
      ...
      <span class="px-3 py-1 rounded-full text-xs font-semibold {statusColors[project.status] ?? 'bg-gray-100 text-gray-600'}">
        {project.status}
      </span>
    </div>
    ...
    <div class="flex gap-3 pt-1 flex-wrap">
      {#if project.url}
        <a href={project.url} target="_blank" rel="noopener"
           class="text-sm px-4 py-1.5 rounded-lg bg-gray-100 hover:bg-gray-200 font-medium">
          🔗 View on Printables
        </a>
      {/if}
      {#if project.editorUrl}
        <a href={project.editorUrl} target="_blank" rel="noopener"
           class="text-sm px-4 py-1.5 rounded-lg bg-gray-100 hover:bg-gray-200 font-medium">
          ✏️ Open Editor
        </a>
      {/if}
    ...
    <div class="space-y-4">
      {#each devlogs as log (log.id)}
        <div class="bg-white rounded-xl border p-4 space-y-2">
          {#if log.image}
            <img src={log.image} alt="devlog" class="w-full h-48 object-cover rounded-lg" />
          {/if}
          ...
          {#if currentUserId}
            <form
              ...
              action="?/addComment"
              use:enhance={() => {
                return ({ update }) => { commentText = ''; selectedDevlogId = null; update(); };
              }}
            ...
            >
              ...
              <input
                ...
                class="flex-1 text-sm border rounded-lg px-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-black"
                required
              />
              <button type="submit"
                class="text-sm px-3 py-1.5 bg-black text-white rounded-lg hover:bg-gray-800">
                Post
              </button>
            </form>
          {/if}
    ...
    <div class="space-y-3">
      {#each comments as c (c.id)}
        <div class="bg-white rounded-xl border p-4">
          ...
          <p class="text-sm text-gray-500 text-xs mb-1">on: "{c.devlogDescription?.slice(0, 60)}…"</p>
