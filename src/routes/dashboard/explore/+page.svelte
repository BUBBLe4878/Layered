...
export async function load({ url, locals }) {
	...
	try {
		...
		// Fetch comments ON the current user's devlogs (if user is logged in)
		let userCommentsData: any[] = [];
		if (locals.user?.id) {
			userCommentsData = await db
				.select({
					id: devlogComment.id,
				...
				.from(devlogComment)
				.innerJoin(devlog, eq(devlog.id, devlogComment.devlogId))
				.where(eq(devlog.userId, locals.user.id))
		...
		const leaderboard = leaderboardRaw
			.map((u) => {
				const hours = (Number(u.totalHours ?? 0))/600;
